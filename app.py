...
# ── Prometheus metrics (NEW — Evidently drift scores) ─────────────────────────
DRIFT_SHARE          = Gauge("evidently_drift_share",
                             "Share of drifted columns (Evidently)")
PREDICTION_DRIFT     = Gauge("evidently_prediction_drift",
                             "1 if prediction drift detected (Evidently)")
GLUCOSE_DRIFT        = Gauge("evidently_glucose_drift",
                             "1 if Glucose drift detected (Evidently)")
BMI_DRIFT            = Gauge("evidently_bmi_drift",
                             "1 if BMI drift detected (Evidently)")
DRIFT_WINDOW_SIZE    = Gauge("evidently_drift_window_size",
                             "Number of predictions in current drift window")
...
# How many predictions to buffer before running Evidently check
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        button[type="submit"] {
            ...
            background: linear-gradient(135deg, var(--coral), var(--purple));
            color: #fff; border: none; border-radius: 14px; padding: 16px 56px;
        ...
        .result-banner {
            margin-top: 28px; border-radius: 16px; padding: 24px 28px;
...
# ── Global state ───────────────────────────────────────────────────────────────
model           = None
reference_data  = None          # training data loaded once at startup
prediction_log  = []            # buffer of incoming feature dicts
...
def load_reference_data():
    """Load training CSV once at startup to use as Evidently reference."""
...
def run_live_drift_check():
    """
    Runs in a background thread every time prediction_log reaches DRIFT_CHECK_EVERY.
    Computes Evidently drift report and pushes results to Prometheus Gauges.
    Also saves the latest HTML report to reports/live_drift_report.html.
    """
    ...
    with drift_lock:
        if reference_data is None or len(prediction_log) < DRIFT_CHECK_EVERY:
            return
        current_df = pd.DataFrame(prediction_log[-DRIFT_CHECK_EVERY:], columns=FEATURES)
    ...
    try:
        report = Report(metrics=[
            ...
            ColumnDriftMetric(column_name="Pregnancies"),
            DataDriftPreset(),
        ])
        report.run(reference_data=reference_data, current_data=current_df)

        # ── Push scalar scores to Prometheus ──
        ...
        for m in metrics_list:
            ...
            if mtype == "DatasetDriftMetric":
                share = res.get("share_of_drifted_columns", 0)
                DRIFT_SHARE.set(share)
                print(f"[Evidently] drift_share={share:.3f}")

            elif mtype == "ColumnDriftMetric":
                col      = res.get("column_name", "")
                detected = int(res.get("drift_detected", False))
                if col == "Glucose":
                    GLUCOSE_DRIFT.set(detected)
                elif col == "BMI":
                    BMI_DRIFT.set(detected)
        ...
        print("[Evidently] Live drift report updated → reports/live_drift_report.html")
...
def drift_report():
    """
    Serves the latest Evidently HTML drift report directly in the browser.
    This is the report generated from live production predictions.
    Falls back to the training-time report if live report not yet generated.
    """
    live_path  = "reports/live_drift_report.html"
    train_path = "reports/data_drift_report.html"
    ...
    if os.path.exists(live_path):
        ...
        return (
            ...
            "📊 No drift report yet.<br><small>Make at least "
            f"{DRIFT_CHECK_EVERY} predictions to generate a live report.</small></h2>",
            404,
        )
...
def drift_summary():
    """Returns the latest Evidently summary as JSON — useful for alerting."""
...
def _log_prediction(source):
    """Appends incoming feature values to the prediction log buffer."""
    global prediction_log
    try:
